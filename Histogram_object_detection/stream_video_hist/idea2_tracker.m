function [x_center, y_center, area_found] = idea2_tracker(varargin)

dstep_row = varargin{1};
dstep_acr = varargin{2};
h_small = varargin{3};
w_small = varargin{4};
rows = varargin{5};
cols = varargin{6};
...
U = varargin{9};
org = varargin{10};
num_of_frames = varargin{11};
rect2 = varargin{12};

if length(varargin) > 12
    x_center = varargin{13};
    y_center = varargin{14};
    area_found = varargin{15};
end
...
% ---------------
    ...
    scale_xmin = ((length(1:dstep_acr:cols))*rect2(1,1))/Uroi_w_small;
    scale_ymin = ((length(1:dstep_row:rows))*rect2(1,2))/Uroi_h_small;

    scalefactor_x = round((w_small-aw-1)/rect2(1, 3));
    scalefactor_y = round((h_small-a-1)/rect2(1, 4));
    scalefactor_x2 = ((length(1:dstep_acr:(w_small-a)))*rect2(1,3))/Uroi_w_small;     % original size
    scalefactor_y2 = ((length(1:dstep_row:(h_small-aw)))*rect2(1,4))/Uroi_h_small;
