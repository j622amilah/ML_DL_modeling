...
public class VehicularMainApp {
	...
	public static void main(String[] args) {
		...
		if (args.length == 5){
			...
			configFile = "C:\\Users\\jafouche\\Documents\\Edge_computing\\EdgeCloudSim\\scripts\\sample_app5\\config\\" + configName + "_config.properties";
			applicationsFile = "C:\\Users\\jafouche\\Documents\\Edge_computing\\EdgeCloudSim\\scripts\\sample_app5\\config\\applications.xml";
			edgeDevicesFile = "C:\\Users\\jafouche\\Documents\\Edge_computing\\EdgeCloudSim\\scripts\\sample_app5\\config\\edge_devices.xml";
			outputFolder = "C:\\Users\\jafouche\\Documents\\Edge_computing\\EdgeCloudSim\\sim_results\\ite" + iterationNumber;
		}
		...
		//String wekaModelsFolder = configFile.substring(0, configFile.lastIndexOf('/')) + "/weka/";  // Relative paths do not work
		// Use absolute paths
		String wekaModelsFolder = "C:\\Users\\jafouche\\Documents\\Edge_computing\\EdgeCloudSim\\scripts\\sample_app5\\config\\weka\\";
		
		// Load initial models: Option to retrain or use existing model
		String which_way = "use_existing";  // "train", "use_existing"
		if (which_way == "train") {
			//
			// Train the initial models using data from the previous simulation
			try {
				DataWrapper.getInstance().train_models(wekaModelsFolder);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
		else { // "use_existing"
			// Calls WekaWrapper.java one time to start the program (it looks for certain data
			// from the CloudSim simulation and predicts a classification and regression result
			// using pre-trained models - it outputs )
			// WekaWrapper.getInstance().initialize("MultilayerPerceptron", "LinearRegression", wekaModelsFolder);
			DataWrapper.getInstance().initialize("MultilayerPerceptron", wekaModelsFolder);
		}
		
		
		// ***SimSettings.java allows us to set the simulation variables
	...
	public static void mainHelper(String outputFolder, String simulationScenario, String orchestratorPolicy, int iterationNumber, int numOfMobileDevice){
		...
		try
		{
			...
			// This is how your call the manager to run a method that you created
			...
			// *******************
			System.out.printf("orchestratorPolicy\n");
			System.out.println(orchestratorPolicy);
			// *******************
