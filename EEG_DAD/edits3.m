...
if rgview.range > dad.data_eegch_dad10art_full_time(end),
    rgview.range = dad.data_eegch_dad10art_full_time(end);
end

if (rgview.start + rgview.range) >= dad.data_eegch_dad10art_full_time(end) 
    rgview.start = dad.data_eegch_dad10art_full_time(end) - rgview.range;
end
...
cond_rg.data = dad.data_eegch_dad10art_full(subrange_idx(dad.data_eegch_dad10art_full_time, rgview.start, rgview.start + rgview.range));
...
% Slider
rem = dad.data_eegch_dad10art_full_time(end) - rgview.range;
if rem <= 0,
    rem = 2;
end
